...
import { Sparkles, ShieldCheck, UserCheck, CheckCircle, BookOpen, Bookmark } from 'lucide-react';
import { HEALER_MANIFESTO, SITE_CONFIG, AUTHOR_BOOKS } from '../data/content';
  ...
  return (
    <section id="hakkimda" className="relative py-14 sm:py-16 bg-[#0D111D] overflow-hidden">
      <div className="w-full max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-12">
        ...
        {/* Dedicated Books Showcase Section */}
        <div className="mt-16 sm:mt-20 pt-12 border-t border-purple-500/20">
          
          {/* Header */}
          <div className="text-center max-w-3xl mx-auto space-y-3 mb-10">
            <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-purple-950/40 border border-purple-400/30 text-purple-200 text-xs font-semibold backdrop-blur-md">
              <BookOpen className="w-3.5 h-3.5 text-amber-300" />
              <span>YAZAR & ESERLERİ</span>
            </div>
            <h3 className="text-2xl sm:text-3xl font-serif font-bold text-white tracking-tight">
              Osman Özden'in Yayınlanan Kitapları
            </h3>
            <p className="text-slate-300 text-xs sm:text-sm font-light leading-relaxed">
              Kadim bilgelik, semboller ve kişisel farkındalık alanındaki derin araştırmalarını ve deneyimlerini kaleme aldığı basılı eserleri.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8 max-w-5xl mx-auto">
            {AUTHOR_BOOKS.map((book) => (
              <div
                key={book.id}
                className="group relative rounded-3xl p-6 sm:p-7 glass-panel bg-[#141A2C]/90 border border-purple-500/20 hover:border-purple-400/50 shadow-xl hover:shadow-purple-500/10 transition-all duration-500 flex flex-col sm:flex-row items-center sm:items-start gap-6 overflow-hidden"
              >
                {/* Background ambient glow */}
                <div className="absolute -top-16 -right-16 w-36 h-36 bg-purple-600/10 rounded-full blur-2xl pointer-events-none group-hover:bg-purple-600/20 transition-colors"></div>

                {/* 3D Book Cover Presentation */}
                <div className="relative flex-shrink-0 w-32 sm:w-36 aspect-[2/3] rounded-xl overflow-hidden shadow-[0_12px_30px_rgba(0,0,0,0.8)] border border-slate-700/60 group-hover:scale-105 group-hover:-rotate-1 transition-all duration-500 bg-slate-900">
                  <img
                    src={book.coverImage}
                    alt={book.title}
                ...
                <div className="flex-1 space-y-3 text-center sm:text-left">
                  <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2">
                    {book.badge && (
                      <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md text-[11px] font-medium bg-purple-500/20 text-purple-300 border border-purple-400/30">
                        <Bookmark className="w-3 h-3 text-purple-400" />
                        {book.badge}
                      </span>
                    )}
                    <span className="px-2.5 py-0.5 rounded-md text-[11px] font-medium bg-amber-500/10 text-amber-300 border border-amber-400/20">
                      {book.publisher}
                    </span>
                  </div>

                  <div>
                    <h4 className="text-lg sm:text-xl font-serif font-bold text-white group-hover:text-purple-200 transition-colors">
                      {book.title}
                    </h4>
                    {book.subtitle && (
                    ...
                    <p className="text-[11px] text-slate-400 mt-1">
                      Yazar: <span className="text-slate-200 font-medium">{book.author}</span>
                    </p>
                  </div>
